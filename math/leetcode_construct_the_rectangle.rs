...
fn main() {
    ...
    assert_eq!(vec![2 ,1], Solution::construct_rectangle(2));
    assert_eq!(vec![3, 2], Solution::construct_rectangle(6));
}
...
impl Solution {
    pub fn construct_rectangle(area: i32) -> Vec<i32> {
        for i in (1..=(area as f64).sqrt() as i32).rev() {
            if area % i == 0 {
                let l: i32 = std::cmp::max(i, area / i);
                let w: i32 = std::cmp::min(i, area / i);
                return vec![l, w];
            }
        }
        return vec![];
