...
class test_128 {
	...
	public static void printNum(int n) {
		...
		if (printHundred(million)) System.out.print("Million ");
		int thousand = (n % 1000000) / 1000;
		if (printHundred(thousand)) System.out.print("Thousand ");
		printHundred(n % 1000);
		System.out.println("");
	}
	...
	public static boolean printHundred(int n) {
		...
		if (hundred > 0) {
			...
			print = true;
			n -= hundred * 100;
			if (n > 0) System.out.print("and ");
		}
		if (n > 0) {
			int two = n / 10;
			int one = n % 10;
			if (two == 0) System.out.print(wordarr[one]);
			else if (two == 1) System.out.print(wordarr1[one]);
		 	else System.out.print(wordarr10[two] + wordarr[one]);
			print = true;
		}
		return print;
	}
