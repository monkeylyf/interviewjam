...
fn main() {
    assert_eq!(vec![true, false, true], Solution::kids_with_candies(vec![12, 1, 12], 10));
}
...
impl Solution {
    pub fn kids_with_candies(candies: Vec<i32>, extra_candies: i32) -> Vec<bool> {
        ...
        let max_value: i32 = *max.unwrap();
        for i in 0..candies.len() {
            let can_be_greatest = candies[i] + extra_candies >= max_value;
            ret.push(can_be_greatest);
        }
